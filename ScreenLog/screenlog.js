/**
 * a screen logger for h5 dev,we can show the console in the phone screen
 */
(function () {
	...
	function createPanel() {
		var div = createElement('div', 'z-index:100000;font-family:Helvetica,Arial,sans-serif;font-size:15px;font-weight:bold;padding:5px;text-align:left;opacity:0.8;position:fixed;right:0;top:0;min-width:30vw;max-width:70vw;max-height:50vh;overflow:auto;background:' + _options.bgColor + ';' + _options.css);
		div.id = 'logger';
		return div;
	}
    ...
    function isShow(flag) {
        localStorage.getItem("screenlog_ball") == "true" ? flag = true : flag = false;
        var ball = $('.col1');
    ...
    function initTouchBall(callback) {
        var touch_ball = '<div class="barrage" id="barrage" style="position:fixed;display:block;z-index: 100001;top:0;">'
            +'<div class="barrage_name" id="barrage_name" style="width:50px;height:50px;background:#00aeff;border-radius:50%;">'
            +'<span class="col1" style="color:#fff;display: block;padding: 7px;font-size: 15px;line-height: 20px;text-align: center;">关闭调试</span>'
            +'</div>'
            +'</div>';
        var body=$("body");
        ...
        var isOpen = true;
        localStorage.getItem("screenlog_ball") == "true" ? isOpen = true : isOpen = false;
        callback(isOpen);
        ...
        cont.on({//绑定事件
            ...
            touchend:function(e) {
                if(singleClick){
                    ...
                    localStorage.setItem("screenlog_ball",isOpen);
                    callback(isOpen);
                }else{
                    console.log("移动事件");
                }
