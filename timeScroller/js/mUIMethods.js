...
function uiMethods() {
    ...
    function initUI(jsonData) {
        if (jsonData.power == 0){
            ...
            $("#powerName")[0].innerHTML = jsonData.name;
            ...
            $("#errorName")[0].innerHTML = jsonData.name;

            //故障解析方法
            errorCodeHandler(jsonData.er);
        }else if(jsonData.runState ==1 || jsonData.stage == 5){
            ...
            //在运行状态点击暂停后，此时runState == 0,但是不点击返回依旧不能显示
            //控制界面，需要设置一个标志位
        ...
        switch (obj.id) { //此处传来的obj即为界面控件,id需要跟html对应
            ...
            case "adv_dry2":
                if (state == -1){
                    ...
                    $("#adv_dry1").css({"background-image":"url(img/adv_nightWash_unset.png)"});
                    $("#adv_dry2").css({"background-image":"url(img/adv_nightWash_unset.png)"});
                }else if (state == 1){
                    $("#adv_dry1").css({"background-image":"url(img/adv_nightWash_set.png)"});
                    $("#adv_dry2").css({"background-image":"url(img/adv_nightWash_unset.png)"});
                }else if (state == 2){
                    $("#adv_dry1").css({"background-image":"url(img/adv_nightWash_unset.png)"});
                    $("#adv_dry2").css({"background-image":"url(img/adv_nightWash_set.png)"});
                }
                break;
